...
STATE_RESERVATION_STOP_TIME = ["StopTime", "airState.reservation.targetTimeToStop"]
STATE_RESERVATION_START_TIME = ["StartTime", "airState.reservation.targetTimeToStart"]
...
STATE_AIRPOLUTION_PM1 = ["SensorPM1","airState.quality.PM1"]
STATE_AIRPOLUTION_PM2 = ["SensorPM2","airState.quality.PM2"]
STATE_AIRPOLUTION_PM10 = ["SensorPM10","airState.quality.PM10"]
...
CMD_RESERVATION_START_TIME = [CTRL_BASIC, "Set", STATE_RESERVATION_START_TIME]
...
class ACFanSpeedRAC(Enum):
    ...
    약중풍 = "@AC_MAIN_WIND_STRENGTH_LOW_MID_W"
    중풍 = "@AC_MAIN_WIND_STRENGTH_MID_W"
    중강풍 = "@AC_MAIN_WIND_STRENGTH_MID_HIGH_W"
...
class ACFanSpeedPAC(Enum):
    ...
    정지_중풍 = "@AC_MAIN_WIND_STRENGTH_MID_RIGHT_W"
    정지_강풍 = "@AC_MAIN_WIND_STRENGTH_HIGH_RIGHT_W"
    약풍_정지 = "@AC_MAIN_WIND_STRENGTH_LOW_LEFT_W"
    중풍_정지 = "@AC_MAIN_WIND_STRENGTH_MID_LEFT_W"
    강풍_정지 = "@AC_MAIN_WIND_STRENGTH_HIGH_LEFT_W"
    
    약풍_약풍 = "@AC_MAIN_WIND_STRENGTH_LOW_LEFT_W|AC_MAIN_WIND_STRENGTH_LOW_RIGHT_W"
    약풍_중풍 = "@AC_MAIN_WIND_STRENGTH_LOW_LEFT_W|AC_MAIN_WIND_STRENGTH_MID_RIGHT_W"
    약풍_강풍 = "@AC_MAIN_WIND_STRENGTH_LOW_LEFT_W|AC_MAIN_WIND_STRENGTH_HIGH_RIGHT_W"
    중풍_약풍 = "@AC_MAIN_WIND_STRENGTH_MID_LEFT_W|AC_MAIN_WIND_STRENGTH_LOW_RIGHT_W"
    중풍_중풍 = "@AC_MAIN_WIND_STRENGTH_MID_LEFT_W|AC_MAIN_WIND_STRENGTH_MID_RIGHT_W"
    중풍_강풍 = "@AC_MAIN_WIND_STRENGTH_MID_LEFT_W|AC_MAIN_WIND_STRENGTH_HIGH_RIGHT_W"
    강풍_약풍 = "@AC_MAIN_WIND_STRENGTH_HIGH_LEFT_W|AC_MAIN_WIND_STRENGTH_LOW_RIGHT_W"
...
class AirConditionerDevice(Device):
    ...
    def fan_speeds(self):
        ...
        if self.model_info.model_type == "PAC":
            return self._get_property_values(SUPPORT_WIND_STRENGTH, ACFanSpeedPAC)
        else:
            return self._get_property_values(SUPPORT_WIND_STRENGTH, ACFanSpeedRAC)
...
#        return self._is_pm_supported[2]
    ...
    @cached_property
    def start_time_range(self) -> list[int]:
        """Return valid range for start time."""
        key = self._get_state_key(STATE_RESERVATION_START_TIME)
        if (range_val := self.model_info.value(key, TYPE_RANGE)) is None:
            return [0, 1440]
        return [range_val.min, range_val.max]

    @property
    def is_reservation_start_time_available(self) -> bool:
        """Return if reservation start time is available."""
        if (status := self._status) is None:
            return False
        if (
            status.device_features.get(AirConditionerFeatures.RESERVATION_START_TIME)
            is None
        ):
            return False
        return status.is_on and (
    ...
    async def set_reservation_start_time(self, value: int):
        """Set the device start time reservation in minutes."""
        if not self.is_reservation_start_time_available:
            raise ValueError("Reservation start time is not available")
        valid_range = self.start_time_range
        if not (valid_range[0] <= value <= valid_range[1]):
            raise ValueError(
                f"Invalid start time value. Valid range: {valid_range[0]} - {valid_range[1]}"
            )
        keys = self._get_cmd_keys(CMD_RESERVATION_START_TIME)
        await self.set(keys[0], keys[1], key=keys[2], value=str(value))
...
#        return value == MODE_ON
    ...
    @property
    def reservation_start_time(self):
        """Return reservation start time in minutes."""
        key = self._get_state_key(STATE_RESERVATION_START_TIME)
        if (value := self.to_int_or_none(self.lookup_range(key))) is None:
            return None
        return self._update_feature(
            AirConditionerFeatures.RESERVATION_START_TIME, value, False
        )
